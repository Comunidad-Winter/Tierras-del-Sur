...
On Error GoTo fin
    ...
    Set Segs = Loader.LoadSegment(Archivo)
...
If IsPlayingCheck Then
     ...
     Segs.SetStartPoint (0)
     Call Perf.Stop(Segs, SegState, 0, 0)
     'Seg.Unload
     Call Perf.Reset(0)
End If
End Sub
...
If Musica = 1 Then Exit Sub
On Error GoTo fin
    Set SegState = Perf.PlaySegment(Segs, 0, 0)
    IsPlayingCheck = True
    Exit Sub
fin:
...
Sonando = (Perf.IsPlaying(Segs, SegState) = True)
End Function
