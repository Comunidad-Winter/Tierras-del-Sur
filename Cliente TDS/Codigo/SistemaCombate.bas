...
If Npclist(NpcIndex).Stats.MinHP <= 0 Then
          ...
          If Npclist(NpcIndex).NPCtype = DRAGON Then
            ...
            Call LogDesarrollo(UserList(UserIndex).Name & " quemo una matadragones")
          End If
...
If UserList(VictimIndex).flags.Privilegios >= 1 Then
    SendData ToIndex, AttackerIndex, 0, "Y181"
    PuedeAtacar = False
    Exit Function
End If
...
' marche 10-4 ' lo de las partyss para que acumule la exp y dsp la divide mas adelante
