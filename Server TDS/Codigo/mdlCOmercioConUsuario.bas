...
If UserList(Origen).ComUsu.DestUsu = Destino And _
   UserList(Destino).ComUsu.DestUsu = Origen Then
    ...
    If UserList(Origen).flags.Privilegios > 1 Then
        Call LogGM(UserList(Origen).Name, " Ha mantenido comercio seguro con " & UserList(Destino).Name, False)
    End If
    If UserList(Destino).flags.Privilegios > 1 Then
        Call LogGM(UserList(Destino).Name, " Ha mantenido comercio seguro con " & UserList(Origen).Name, False)
    End If
    '[Wizard]
Else
