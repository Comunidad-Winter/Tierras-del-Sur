...
'[Misery_Ezequiel 26/06/05]
        If Arma.SubTipo = MATADRAGONES Then ' Usa la matadragones?
            ...
            Else ' Sino es dragon daño es 0
                Call Senddata(ToIndex, UserIndex, 0, "Y347")
                DañoArma = 0
                DañoMaxArma = 0
                Exit Function
            End If
...
Public Sub RestarCriminalidad(ByVal UserIndex As Integer)
    ...
    UserList(UserIndex).Reputacion.BandidoRep = UserList(UserIndex).Reputacion.BandidoRep - vlASALTO
        ...
        Call Senddata(ToMap, UserIndex, UserList(UserIndex).Pos.Map, "CHC," & UserList(UserIndex).Char.charindex)
        End If
    End If
...
For j = 1 To MAXMASCOTAS
    If UserList(UserIndex).MascotasIndex(j) > 0 Then
        If UserList(UserIndex).MascotasIndex(j) <> NpcIndex Then
            '[Wizard 03/09/05]=> Si es el ele de fuego de tierra no ataca npcs, esto se podria hacer con una variable desde los dats, pero me parece q es medio al pedo total se toma solo en este sub.
            If Npclist(UserList(UserIndex).MascotasIndex(j)).Numero = 93 Or Npclist(UserList(UserIndex).MascotasIndex(j)).Numero = 94 Then GoTo 1
            '[/Wizard]
...
If Npclist(NpcIndex).CanAttack = 1 And UserList(UserIndex).flags.Mimetizado = 0 Then
...
'[Wizard] Si es una mascota, entonce le da experiencia al maestro por cada bife que le pone.
...
If Distancia(UserList(UserIndex).Pos, Npclist(NpcIndex).Pos) > MAXDISTANCIAARCO Then
   Call Senddata(ToIndex, UserIndex, 0, "Y6")
   Exit Sub
End If
' marche

If Npclist(NpcIndex).NPCtype = NPCTYPE_GUARDIAS And UserList(UserIndex).flags.Seguro = True Then
Call Senddata(ToIndex, UserIndex, 0, "Y379")
Exit Sub
End If

If UserList(UserIndex).flags.Seguro = True And Npclist(NpcIndex).MaestroUser <> 0 Then
    If Not Criminal(Npclist(NpcIndex).MaestroUser) Then
        Call Senddata(ToIndex, UserIndex, 0, "Y171")
        Exit Sub
    End If
End If
...
If UserList(UserIndex).Faccion.ArmadaReal = 1 And Npclist(NpcIndex).MaestroUser <> 0 Then
    If Not Criminal(Npclist(NpcIndex).MaestroUser) Then
        Call Senddata(ToIndex, UserIndex, 0, "Y172")
        Exit Sub
    End If
End If
...
End If
 'Call Desarmar(AtacanteIndex, VictimaIndex) ¿¿ que hace esto aca?? :S:S:S
'[\]Marche 12/07/05]
       ...
       '[eLwE 19/05/05]Comenta que hiciste O_O
            ...
            If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).MaestroUser > 0 Then
                If MapInfo(Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Pos.Map).Pk = False Then
                    Call Senddata(ToIndex, UserIndex, 0, "Y173")
                    Exit Sub
                End If
                If UserList(UserIndex).flags.Seguro = True And Not Criminal(Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).MaestroUser) Then
                    Call Senddata(ToIndex, UserIndex, 0, "Y12")
                    Exit Sub
                End If
            End If
            Call UsuarioAtacaNpc(UserIndex, MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex)
        Else
            Call Senddata(ToIndex, UserIndex, 0, "Y174")
        End If
       '[\]eLwE 19/05/05]
...
If Distancia(UserList(AtacanteIndex).Pos, UserList(VictimaIndex).Pos) > MAXDISTANCIAARCO Then
   Call Senddata(ToIndex, AtacanteIndex, 0, "Y6")
   Exit Sub
End If
...
If UsuarioImpacto(AtacanteIndex, VictimaIndex) Then
    ...
    If UserList(VictimaIndex).flags.Navegando = 0 And UserList(VictimaIndex).flags.Meditando = False Then Call Senddata(ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CFX" & UserList(VictimaIndex).Char.charindex & "," & FXSANGRE & "," & 0)
...
End If
If UserList(VictimaIndex).Counters.Saliendo = True Then
...
If UserList(AtacanteIndex).flags.Hambre = 0 And UserList(AtacanteIndex).flags.Sed = 0 Then
        ...
        'sino tal vez lucha libre
                Call SubirSkill(AtacanteIndex, Wresterling)
        End If
        '[Wizard 03/09/05] => Bug viejisimo del ao que el q pega subia tacticas
...
Sub UsuarioAtacadoPorUsuario(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer)
      ...
      If UserList(VictimIndex).flags.Meditando = True Then
            UserList(VictimIndex).flags.Meditando = False
            UserList(VictimIndex).Counters.bPuedeMeditar = False
            Call Senddata(ToIndex, VictimIndex, 0, "Y377")
            Call Senddata(ToIndex, VictimIndex, 0, "MEDOK")
     End If
        
If UserList(VictimIndex).Counters.Saliendo = True Then
...
Dim t As eTrigger6
If UserList(AttackerIndex).flags.ModoCombate = False Then
    Senddata ToIndex, AttackerIndex, 0, "Y218"
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(VictimIndex).flags.Muerto = 1 Then
    Senddata ToIndex, AttackerIndex, 0, "Y175"
    PuedeAtacar = False
    Exit Function
End If
...
If MapInfo(UserList(VictimIndex).Pos.Map).Pk = False Then
    Call Senddata(ToIndex, AttackerIndex, 0, "Y176")
    PuedeAtacar = False
    Exit Function
End If
...
If UserList(AttackerIndex).flags.Seguro Then
        If Not Criminal(VictimIndex) Then
                Call Senddata(ToIndex, AttackerIndex, 0, "Y12")
                Exit Function
        End If
End If
PuedeAtacar = True
...
If Npclist(NpcIndex).MaestroUser > 0 Then
    If MapInfo(UserList(AttackerIndex).Pos.Map).Pk = False Then
      Call Senddata(ToIndex, AttackerIndex, 0, "||No puedes atacar mascotas en zonas seguras." & FONTTYPE_FIGHT)
    Exit Function
    End If
    
'If Npclist(NpcIndex).Stats.Alineacion = 0 And UserList(AttackerIndex).flags.Seguro = False Then
 '   Call Senddata(ToIndex, AttackerIndex, 0, "Y379")
  '  Exit Function
'End If
...
End If
If UserList(AttackerIndex).flags.Muerto = 1 Then
    Senddata ToIndex, AttackerIndex, 0, "||No podés atacar porque estas muerto." & FONTTYPE_INFO
    PuedeAtacarNPC = False
    Exit Function
End If
If UserList(AttackerIndex).flags.Privilegios = 1 Then
