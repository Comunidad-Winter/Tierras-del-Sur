...
If InMapBounds(Map, X, Y) Then
    ...
    If MapData(Map, X, Y).TileExit.Map > 0 Then
        ...
        If UCase$(MapInfo(MapData(Map, X, Y).TileExit.Map).Restringir) = "SI" Then
            '¿El usuario es un newbie? [WizArD]
            If EsNewbie(UserIndex) Or UserList(UserIndex).flags.Privilegios > 0 Then
            ...
            Exit Sub
            End If
'[Misery_Ezequiel 26/05/05]
        '¿Es mapa de lvl 0?
        If MapInfo(MapData(Map, X, Y).TileExit.Map).Nivel = 0 Then
           If LegalPos(MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, Y).TileExit.Y, PuedeAtravesarAgua(UserIndex)) Then
                    If FxFlag Then '¿FX?
                        Call WarpUserChar(UserIndex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, Y).TileExit.Y, True)
                    Else
                ...
                End If
            End If
            '¿Es mapa para powas (Nivel > 0 )?
        If MapInfo(MapData(Map, X, Y).TileExit.Map).Nivel Then
            '¿El usuario es powa (Nivel => MapInfo(MapData(Map, X, Y).TileExit.Map).Nivel)? '=> Ahora los GMs entran a cualquier mapa, [WiZaRD]
            If UserList(UserIndex).Stats.ELV >= MapInfo(MapData(Map, X, Y).TileExit.Map).Nivel Or UserList(UserIndex).flags.Privilegios > 0 Then
                If LegalPos(MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, Y).TileExit.Y, PuedeAtravesarAgua(UserIndex)) Then
                    If FxFlag Then '¿FX?
                        Call WarpUserChar(UserIndex, MapData(Map, X, Y).TileExit.Map, MapData(Map, X, Y).TileExit.X, MapData(Map, X, Y).TileExit.Y, True)
                    Else
            ...
            Else 'No es powa
                ...
                Call ClosestLegalPos(UserList(UserIndex).Pos, nPos)
                '[\]Misery_Ezequiel 26/06/05]
                Call Senddata(ToIndex, UserIndex, 0, "Y276")
                If nPos.X <> 0 And nPos.Y <> 0 Then
                        Call WarpUserChar(UserIndex, nPos.Map, nPos.X, nPos.Y)
                End If
            End If
...
Sub ClosestLegalPos(Pos As WorldPos, ByRef nPos As WorldPos, Optional ByVal AguaValida As Byte)
'*****************************************************************
...
Function LegalPos(ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, Optional ByVal PuedeAgua As Byte) As Boolean
'¿Es un mapa valido?
...
Else
 ...
 Else
   LegalPosNPC = (MapData(Map, X, Y).Blocked <> 1) And _
     ...
     (MapData(Map, X, Y).NpcIndex = 0) And _
     (MapData(Map, X, Y).trigger <> TRIGGER_POSINVALIDA) _
     And HayAgua(Map, X, Y) '[Wizard 07/09/05] Esto evita que los npc marinos spawndeen adentro de una isla.
 End If
End If
End Function
...
If UserList(TempCharIndex).flags.AdminInvisible = 0 Or UserList(UserIndex).flags.Privilegios > 1 Then
 ...
 'Si esta muerto solo manda que esta muerto! re pocos if y le pocos bytes! que pro que soy!
    ...
    If UserList(TempCharIndex).flags.Muerto = 1 Then
        ...
        Stat = "0"
        End If
        Call Senddata(ToIndex, UserIndex, 0, "|2" & UserList(TempCharIndex).Name & "," & Stat)
        FoundSomething = 1
...
'Como funciona esto?
'Manda:
'NEWBIE,ARMADA RANGO CONSEJO, CAOS RANGO CONSEJO,CLAN,DESC
'Dios tengo que agregar esto que feo
Dim Ranginius As Integer
Dim rangi As Integer
If UserList(TempCharIndex).Faccion.ArmadaReal > 0 Then
rangi = UserList(TempCharIndex).Faccion.RecompensasReal
If rangi >= 10 Then rangi = 9
Else
    If UserList(TempCharIndex).Faccion.FuerzasCaos > 0 Then
    rangi = UserList(TempCharIndex).Faccion.RecompensasCaos
    If rangi >= 10 Then rangi = 9
    End If
End If


Stat = Stat & UserList(TempCharIndex).Faccion.ArmadaReal & rangi & UserList(TempCharIndex).flags.PertAlCons & "," & UserList(TempCharIndex).Faccion.FuerzasCaos & rangi & UserList(TempCharIndex).flags.PertAlConsCaos & "," & UserList(TempCharIndex).GuildInfo.GuildName & "," & UserList(TempCharIndex).Desc

'Aqui manda si es gm consejero cimi o lo que sea.
...
End If
    ...
    If FoundChar = 2 Then '¿Encontro un NPC?
           ...
           If UserList(UserIndex).Stats.UserSkills(Supervivencia) >= 60 Then
            ...
            If Len(Npclist(TempCharIndex).Desc) > 1 Then
                Call Senddata(ToIndex, UserIndex, 0, "||" & vbWhite & "°" & Npclist(TempCharIndex).Desc & "°" & Npclist(TempCharIndex).Char.charindex)
            Else
               '[Wizard 04/09/05]
                If Npclist(TempCharIndex).MaestroUser > 0 Then
                    Call Senddata(ToIndex, UserIndex, 0, "||" & Npclist(TempCharIndex).Name & " es mascota de " & UserList(Npclist(TempCharIndex).MaestroUser).Name & estatus & IIf(Npclist(TempCharIndex).flags.Paralizado = 1, " [Paralizado]", IIf(Npclist(TempCharIndex).flags.Inmovilizado = 1, " [Inmovilizado]", "")) & FONTTYPE_INFO)
                Else
                    Call Senddata(ToIndex, UserIndex, 0, "||" & Npclist(TempCharIndex).Name & "." & estatus & IIf(Npclist(TempCharIndex).flags.Paralizado = 1, " [Paralizado]", IIf(Npclist(TempCharIndex).flags.Inmovilizado = 1, " [Inmovilizado]", "")) & FONTTYPE_INFO)
                End If
            End If
               '[/Wizard]
            FoundSomething = 1
