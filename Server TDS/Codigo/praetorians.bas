...
On Error GoTo errorh
    ...
    Call Senddata(ToMap, 0, Npclist(npcorig).Pos.Map, "M1" & Npclist(npcorig).Char.charindex)
...
On Error GoTo errorh
    ...
    NPCy = Npclist(npcorig).Pos.Y
    'Call SendData(ToMap, 0, Npclist(npcorig).Pos.Map, "MP" & Npclist(npcorig).Char.charindex & "," & NPCx & "," & NPCy - 1)
    Call Senddata(ToMap, 0, Npclist(npcorig).Pos.Map, "M3" & Npclist(npcorig).Char.charindex)
    'Update map and npc pos
    MapData(mapa, NPCx, NPCy).NpcIndex = 0
    Npclist(npcorig).Pos.Y = NPCy - 1
...
On Error GoTo errorh
    ...
    Call Senddata(ToMap, 0, Npclist(npcorig).Pos.Map, "M4" & Npclist(npcorig).Char.charindex)
    'Update map and npc pos
    MapData(mapa, NPCx, NPCy).NpcIndex = 0
...
On Error GoTo errorh
    ...
    'Call SendData(ToMap, 0, Npclist(npcorig).Pos.Map, "MP" & Npclist(npcorig).Char.charindex & "," & NPCx + 1 & "," & NPCy)
    Call Senddata(ToMap, 0, Npclist(npcorig).Pos.Map, "M2" & Npclist(npcorig).Char.charindex)
    'Update map and npc pos
    MapData(mapa, NPCx, NPCy).NpcIndex = 0
    Npclist(npcorig).Pos.X = NPCx + 1
...
On Error GoTo errorh
    ...
    If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY + 1) Then
        ...
        Call Senddata(ToMap, 0, NPCPosM, "M5" & Npclist(npcind).Char.charindex)
        'Update map and npc pos
    ...
    If LegalPos(NPCPosM, NPCPosX - 1, NPCPosY - 1) Then
        'Call SendData(ToMap, 0, NPCPosM, "MP" & Npclist(npcind).Char.charindex & "," & NPCPosX - 1 & "," & NPCPosY - 1)
        Call Senddata(ToMap, 0, NPCPosM, "M7" & Npclist(npcind).Char.charindex)
        'Update map and npc pos
        MapData(NPCPosM, NPCPosX, NPCPosY).NpcIndex = 0
        Npclist(npcind).Pos.Y = NPCPosY - 1
        Npclist(npcind).Pos.X = NPCPosX - 1
    ...
    If LegalPos(NPCPosM, NPCPosX + 1, NPCPosY - 1) Then
        'Call SendData(ToMap, 0, NPCPosM, "MP" & Npclist(npcind).Char.charindex & "," & NPCPosX + 1 & "," & NPCPosY - 1)
        Call Senddata(ToMap, 0, NPCPosM, "M6" & Npclist(npcind).Char.charindex)
        'Update map and npc pos
        MapData(NPCPosM, NPCPosX, NPCPosY).NpcIndex = 0
        Npclist(npcind).Pos.Y = NPCPosY - 1
        Npclist(npcind).Pos.X = NPCPosX + 1
    ...
    If LegalPos(NPCPosM, NPCPosX - 1, NPCPosY + 1) Then
        'Call SendData(ToMap, 0, NPCPosM, "MP" & Npclist(npcind).Char.charindex & "," & NPCPosX - 1 & "," & NPCPosY + 1)
        Call Senddata(ToMap, 0, NPCPosM, "M8" & Npclist(npcind).Char.charindex)
        'Update map and npc pos
        MapData(NPCPosM, NPCPosX, NPCPosY).NpcIndex = 0
        Npclist(npcind).Pos.Y = NPCPosY + 1
        Npclist(npcind).Pos.X = NPCPosX - 1
