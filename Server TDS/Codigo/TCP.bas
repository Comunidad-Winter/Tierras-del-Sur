...
If UserList(UserIndex).Pos.Map = 0 Then
    ...
    ElseIf UCase$(UserList(UserIndex).Hogar) = "ARGHÂL" Then
        UserList(UserIndex).Pos = Arghâl
    '[\]Misery_Ezequiel 10/07/05]
    End If
Else
   ''TELEFRAG
...
'   End If
   ...
   If CLng(ClientCRC) <> ServerSideCRC Then
     Call Senddata(ToIndex, UserIndex, 0, "ERREl cliente está dañado, por favor descargalo nuevamente desde http://www.aotds.com.ar")
                 ...
                 rs.Close
                  ...
                  End If
                   ...
                   rs.Close
                    ...
                    Else
                        Call Senddata(ToIndex, UserIndex, 0, "ERRSe te ha prohibido la entrada a Tierras Del Sur. Por " & rs!banrazB & ". Para mas información consulta en www.aotds.com.ar")
                        rs.Close
                        Exit Sub
                    End If
                Else
...
'                If Not ValidInputNP(rdata) Then Exit Sub
                ...
                If VersionOK(Ver) Then
                     ...
                     If (UserList(UserIndex).flags.ValCoDe = 0) Or (ValidarLoginMSG(UserList(UserIndex).flags.ValCoDe) <> miinteger) Then
                         Call Senddata(ToIndex, UserIndex, 0, "ERRPara poder continuar con la creación del personaje, debes usar únicamente el cliente oficial de Tierras Del Sur. Bajalo de http://www.aotds.com.ar")
                         'Call LogHackAttemp("IP:" & UserList(UserIndex).ip & " intento crear un bot.")
                         Call CloseSocket(UserIndex)
                         Exit Sub
                     End If
...
Dim josele As Variant
  ...
  Call Senddata(ToAll, 0, 0, "||EL Mesón Hostigado> Escuchadme habitantes de estas tierras." & "~255~131~80~1~1~")
  Call Senddata(ToAll, 0, 0, "||EL Mesón Hostigado> Ois dare una gran recompensa al que me traiga la cabeza del madito " & Amatar & " . " & History & "~255~131~80~1~1~")
  Exit Sub
End If
