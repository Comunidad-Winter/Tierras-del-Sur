...
If Slot > MAX_INVENTORY_SLOTS Then
        ...
        Do Until UserList(UserIndex).Invent.Object(Slot).ObjIndex = 0
            Slot = Slot + 1
            If Slot > MAX_INVENTORY_SLOTS Then
                Call Senddata(ToIndex, UserIndex, 0, "Y21")
                Exit Sub
            End If
        Loop
...
If ObjData(obji).Newbie = 1 Then
    Call Senddata(ToIndex, UserIndex, 0, "Y34")
    Exit Sub
End If
If Npclist(NpcIndex).TipoItems <> OBJTYPE_CUALQUIERA Then
    ...
    If Npclist(NpcIndex).TipoItems <> ObjData(obji).ObjType Then
            Call Senddata(ToIndex, UserIndex, 0, "Y35")
            Exit Sub
    End If
End If
'¿Ya tiene un objeto de este tipo?
...
If Slot <= MAX_INVENTORY_SLOTS Then 'Slot valido
    ...
    If Npclist(NpcIndex).Invent.Object(Slot).Amount + Cantidad <= MAX_INVENTORY_OBJS Then
        ...
        Call SubirSkill(UserIndex, Comerciar)
    Else
        Call Senddata(ToIndex, UserIndex, 0, "Y36")
    End If
Else
    Call QuitarUserInvItem(UserIndex, CByte(ObjIndex), Cantidad)
    'Le sumamos al user el valor en oro del obj vendido
...
If UserList(UserIndex).Stats.GLD >= (val * Cantidad) Then
       ...
       End If
Else
    Call Senddata(ToIndex, UserIndex, 0, "Y37")
    Exit Sub
End If
Exit Sub
errhandler:
...
Call SendUserStatsBox(UserIndex)
If UserList(UserIndex).Invent.Object(Item).Amount > 0 And UserList(UserIndex).Invent.Object(Item).Equipped = 0 And UserList(UserIndex).Invent.Object(Item).ObjIndex <> 12 Then
